...
TInt32 IfServiceCenterStub::OnSerivceStarted(TUInt32 nServiceId,const TChar* pszName,CDoid tDoid)
{
    ...
    CMessageHeader *pMsg = m_pOnwerObj->PrepareMsg(nLen,(SERVICECENTER_SERVICE_ID|IFSERVICECENTER_INTERFACE_ID|ONSERIVCESTARTED_TUINT32_CONST_TCHAR_PT_CDOID_ID),&m_tTarget,1,false);
    if (NULL == pMsg)
    {
        return OUT_OF_MEM;
    }
...
TInt32 IfServiceCenterStub::OnServiceStoped(TUInt32 nServiceId,CDoid tDoid)
{
    ...
    CMessageHeader *pMsg = m_pOnwerObj->PrepareMsg(nLen,(SERVICECENTER_SERVICE_ID|IFSERVICECENTER_INTERFACE_ID|ONSERVICESTOPED_TUINT32_CDOID_ID),&m_tTarget,1,false);
    if (NULL == pMsg)
    {
        return OUT_OF_MEM;
    }
...
TInt32 IfServiceCenterStub::ServiceStatic(TUInt32 uServiceId,CDoid tDoid,TLV<TUInt16,TUInt16> tDatas)
{
    ...
    CMessageHeader *pMsg = m_pOnwerObj->PrepareMsg(nLen,(SERVICECENTER_SERVICE_ID|IFSERVICECENTER_INTERFACE_ID|SERVICESTATIC_TUINT32_CDOID_TLV_TPL_BEGIN_TUINT16_AND_TUINT16_TPL_END__ID),&m_tTarget,1,false);
    if (NULL == pMsg)
    {
        return OUT_OF_MEM;
    }
